...
class MwaSprint4Stack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        waleed_lambda.add_environment(key='sns_arn', value=topic.topic_arn) 



        DBTable = self.create_table("AlarmInfoTable", "AlarmName")
        ...
        api_lambda = self.create_lambda("WALEED_API", 'api_lambda.lambda_handler', "./resources", lambda_role)
        api_lambda.apply_removal_policy(RemovalPolicy.DESTROY)
        
        # Create a DynamoDB Table for URL Info
        url_table = self.create_table("url_table", "URLS")
        
        url_table.apply_removal_policy(RemovalPolicy.DESTROY)
        
        # Set URL Table Name to WH/API Lambda Environment Variable
        api_db_name = url_table.table_name
        api_lambda.add_environment(key='Waleed_API_DB', value=api_db_name)
        waleed_lambda.add_environment(key='Waleed_API_DB', value=api_db_name)
        
        # Create REST API for API Lambda
        app_api = api_.LambdaRestApi(self, 'API-Lambda-Endpoint-MWA', handler=api_lambda, proxy=False)
        app_api.apply_removal_policy(RemovalPolicy.DESTROY)
        
        
        # Create CRUD Ops to API
        urls = app_api.root.add_resource("urls")
        urls.add_method("POST")
        urls.add_method("GET")
        urls.add_method("PATCH")
        urls.add_method("DELETE")
        urls.apply_removal_policy(RemovalPolicy.DESTROY)

        # Print API URL
        self._api_url = CfnOutput(self, 'MWA_API_URL', value=app_api.url, description="API URL used to call API Lambda")
    ...
    def create_table(self, id_, partitionkeyname):
        return db_.Table(self, id_,
            removal_policy =RemovalPolicy.DESTROY,
            
            # https://docs.aws.amazon.com/cdk/api/v1/python/aws_cdk.aws_dynamodb/Attribute.html#aws_cdk.aws_dynamodb.Attribute
            partition_key= db_.Attribute(name=partitionkeyname, type=db_.AttributeType.STRING))